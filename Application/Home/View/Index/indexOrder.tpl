...
<ion-content>
        <div class="list has-header">
            <div class="item item-thumbnail-left" href="#" ng-repeat="order in orders | orderBy:'order_time':true">
                ...
                <a class="button button-small button-balanced" data-ui-sref-opts="{cache: false,reload:true}" data-ui-sref="pay({orderid:order.id})" ng-show="order.toBePaid">立即支付</a>
                <a class="button button-small button-positive" ui-sref="tabs.evaluationing({orderid:order.id})" ng-show="order.toBeEvaluation">评价赢积分</a>
            </div>
        </div>   
